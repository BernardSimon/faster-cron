...
async def test_async_retry_success_history_and_last_result():
    ...
    task = cron.tasks[0]
    await cron._execute_task(task, {"scheduled_at": datetime.now(), "task_name": "flaky"})
...
async def test_async_error_callback_on_final_failure():
    ...
    cron = AsyncFasterCron(log_level=50, max_retries=1, retry_delay=0.01, on_error=on_error)
    ...
    task = cron.tasks[0]
    await cron._execute_task(task, {"scheduled_at": datetime.now(), "task_name": "always_fail"})
