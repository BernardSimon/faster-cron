...
async def main():
    print("="*70)
    print("FasterCron v2.0 完整功能演示（异步模式）")
    print("="*70)
    ...
    cron = AsyncFasterCron(
        ...
        on_error=lambda err, record: print(f"\n❌ 最终失败：{record.task_name} - {err}")
    )
    ...
    async def dynamic_function(ctx):
        dynamic_counter[0] += 1
    ...
    print(f"\n✅ 调度器已停止，总运行时长：{total_duration:.1f} 秒")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("错误：需要安装 asyncio (Python 标准库)")
        exit(1)
